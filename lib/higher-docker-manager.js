...
class HigherDockerManager {
     ...
     * @param {string} name Name of the image to be pulled from Docker registry. This may contain
                ...
                return Promise.all(_.map(containers, (container) => {
                    return container.status()
                        ...
                        .then((containerNetworks) => {
                            return _.some(containerNetworks,
                                (containerNetworkName) => _.some(networks, (networkName) =>
    ...
    static runTemporaryContainer(params) {
        ...
        const scheduleDelayedCleanup = () => {
            setImmediate(() => {
                ...
                temporaryContainer.wait()
                    ...
                    .catch((err) => {
                        console.error('Failed to delete temporary container', err);
        ...
        return docker.container.create(params)
            ...
            .catch((err) => {
                console.error('Failed to start or run temporary container', err);
    ...
    * @return {object} Found object if it was found, otherwise `null`.
    ...
    /**
     * Converts container output buffers into a string.
        ...
        return _
            ...
            .map((buffer) => {
                if (!Buffer.isBuffer(buffer) || buffer.length < PAYLOAD_HEADER_SIZE) {
                    console.error('Bad container output', buffer);
                    return [];
                }
                ...
                if (currentIndex !== buffer.length) {
                    console.warn('Docker buffer not entirely read', buffer);
                }
